<?php
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty ($input_recipientSignature)){
      $recipientSignature_err = "Please enter Recipient Name";
    } elseif(!filter_var($input_recipientSignature, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => "/^[a-zA-Z\s]+$/")))){
      $recipientSignature_err = "Please enter a valid Recipient Name";
    } else {
      $recipientSignature = $input_recipientSignature;
    }
          
    if(empty($dateOrdered_err) && empty($distributorName_err) && empty($itemName_err) && empty($itemQuantity_err) && empty($itemNumber_err) && empty($recipientSignature_err)){
      ...
      if($stmt = $mysqli->prepare($sql)){
        ...
        $param_recipient = $recipientSignature;
